This is a set of Ansible scripts for deploy the autosmart2 Django application.
...
1. Run `vagrant up`
1. There is a final step where we open up [http://autosmart2.local](http://autosmart2.local) in Chrome, and that will surely fail on non-OSX Linux machines, but only after the rest of the command has finished.